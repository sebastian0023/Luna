...
router.get("/api/data", (request, env2) => {
  return new Response(
    JSON.stringify({
...
router.post("/api/data", async (request, env2) => {
  try {
...
router.get("/api/confessions", async (request, env2) => {
  try {
    if (env2.DB) {
      try {
    ...
    return new Response(
      JSON.stringify([
        { _id: "1", author: "Sistema", message: "Ejemplo de confesi\xF3n", createdAt: (/* @__PURE__ */ new Date()).toISOString() }
      ]),
      { headers: { "Content-Type": "application/json" }, status: 200 }
    );
  } catch (error3) {
    console.error("Error fetching confessions:", error3);
    return new Response(
      JSON.stringify({ error: error3.message || "Error desconocido" }),
      { headers: { "Content-Type": "application/json" }, status: 500 }
    );
  }
});
router.post("/api/confessions", async (request, env2) => {
  try {
    let body = {};
    ...
    const author = body.author || "An\xF3nimo";
    if (env2.DB) {
      try {
    ...
    console.error("Error creating confession:", error3);
    return new Response(
      JSON.stringify({ error: error3.message || "Error desconocido" }),
      { headers: { "Content-Type": "application/json" }, status: 500 }
    );
...
router.delete("/api/confessions/:id", async (request, env2) => {
  try {
    ...
    console.error("Error deleting confession:", error3);
    return new Response(
      JSON.stringify({ error: error3.message || "Error desconocido" }),
      { headers: { "Content-Type": "application/json" }, status: 500 }
    );
...
router.all("*", async (request, env2) => {
  if (env2.ASSETS) {
    const response = await env2.ASSETS.fetch(request);
    if (response.status !== 404) {
      return response;
    }
    return await env2.ASSETS.fetch(new Request("http://assets/index.html"));
  }
  return new Response("Not Found", { status: 404 });
});
var src_default = {
  fetch: /* @__PURE__ */ __name((request, env2, ctx) => router.handle(request, env2, ctx), "fetch")
};
...
// .wrangler/tmp/bundle-lHoUwT/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-lHoUwT/middleware-loader.entry.ts
